...
calcular2 = function () {
    ...
    if (esValidaNota(nota1, "lblError1") & esValidaNota(nota2, "lblError2") & esValidaNota(nota3, "lblError3")) {
        resultado = calcularPromedio(nota1, nota2, nota3);
        resultadoFormato = resultado.toFixed(2);
        mostrarTexto("lblResultado", resultadoFormato);
    } else {
...
esValidaNota = function (nota, idComponente) {
    hayErrores = false;
    if (isNaN(nota)) {
        mostrarTexto(idComponente, "SE DEBE AGREGAR UN NUMERO");
        hayErrores = true;
    }
    if (nota < 0 || nota > 10) {
        mostrarTexto(idComponente, "DEBE ESTAR EN UN RANGO DE 0 Y 10");
        hayErrores = true;
    }
    if(hayErrores==false){
